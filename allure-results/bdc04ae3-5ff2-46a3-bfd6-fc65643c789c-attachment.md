...
```
Error: locator.selectOption: Error: strict mode violation: locator('.multiselect-all') resolved to 2 elements:
    1) <li class="multiselect-item multiselect-all">…</li> aka getByRole('listitem').filter({ hasText: 'Select all' })
    2) <a tabindex="0" class="multiselect-all">…</a> aka locator('a').filter({ hasText: 'Select all' })
...
> 7  |   await page.locator('.multiselect-all').selectOption({ label: 'Select all'});
     |                                          ^ Error: locator.selectOption: Error: strict mode violation: locator('.multiselect-all') resolved to 2 elements:
