...
> 4  | await page.goto('https://testautomationpractice.blogspot.com/');
     |            ^ Error: page.goto: Test timeout of 30000ms exceeded.
  5  | await expect(page.locator('#name')).toBeVisible();
  6  | await expect(page.locator('#name')).toBeEditable();
  7  | await expect(page.locator('#name')).toBeEnabled();
  8  | await expect(page.locator('#name')).toBeEmpty();
  9  | await page.locator('#name').fill("Pravin");
