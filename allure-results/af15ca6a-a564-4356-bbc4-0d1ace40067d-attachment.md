...
```
Error: locator.allTextContents: Unsupported token "@class" while parsing css selector ".//div[@class="product-price"]/span/span[@class="product-discountedPrice"] | .//div[contains(@class,"product-price")]//span[not(contains(@class,"strike"))]". Did you mean to CSS.escape it?
```
            ...
            - list [ref=e120]:
              - listitem [ref=e121]:
                - generic [ref=e122] [cursor=pointer]:
                  - text: White
                  - generic [ref=e124]: (18855)
              - listitem [ref=e126]:
                - generic [ref=e127] [cursor=pointer]:
                  - text: Blue
                  - generic [ref=e129]: (9563)
              - listitem [ref=e131]:
                - generic [ref=e132] [cursor=pointer]:
                  - text: Black
                  - generic [ref=e134]: (8042)
              - listitem [ref=e136]:
...
```ts
  ...
  29 |     console.log(priceText);
  30 | 
  31 |     const products = page.locator('//li[contains(@class,"product-base")]');
  32 | const count = await products.count();
  33 | 
  34 | const productData = [];
  35 | 
  36 | for (let i = 0; i < count; i++) {
  37 |     const product = products.nth(i);
  38 | 
  39 |     // Get price (discounted price if present, else the regular price)
  40 |     const priceLocator = product.locator(
  41 |         './/div[@class="product-price"]/span/span[@class="product-discountedPrice"] | .//div[contains(@class,"product-price")]//span[not(contains(@class,"strike"))]'
  42 |     );
  43 | 
> 44 |     const priceTexts = await priceLocator.allTextContents();
     |                                           ^ Error: locator.allTextContents: Unsupported token "@class" while parsing css selector ".//div[@class="product-price"]/span/span[@class="product-discountedPrice"] | .//div[contains(@class,"product-price")]//span[not(contains(@class,"strike"))]". Did you mean to CSS.escape it?
  45 |     if (priceTexts.length === 0) continue;
  46 | 
  47 |     const price = Number(priceTexts[0].replace(/[^0-9]/g, ''));
  48 | 
  49 |     // Get product name — adjust selector to match actual name element
  50 |     const nameLocator = product.locator('.//div[contains(@class,"product-productMetaInfo")]//h3 | .//div[contains(@class,"product-productMetaInfo")]//h4');
  51 |     const name = (await nameLocator.allTextContents())[0]?.trim() || 'Unknown';
  52 | 
  53 |     productData.push({ name, price });
  54 | }
  55 | 
  56 | // Find the product with the minimum price
  57 | const minProduct = productData.reduce((min, curr) => (curr.price < min.price ? curr : min), productData[0]);
  58 | 
  59 | console.log("Minimum price is: " + minProduct.price);
  60 | console.log("Product name: " + minProduct.name);
  61 | console.log(productData);
  62 | 
  63 | });
  64 | 
  65 | 
