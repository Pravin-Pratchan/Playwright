...
```
Error: locator.fill: Error: strict mode violation: locator('input[name="testName"]') resolved to 2 elements:
    1) <input type="text" name="testName" required="required" data-error="Enter the Test" placeholder="Enter the Test *"/> aka getByRole('textbox', { name: 'Enter the Test *' })
    2) <input value="" size="40" type="text" required="" name="testName" aria-required="true" aria-invalid="false" placeholder="Enter Test name"/> aka getByRole('textbox', { name: 'Test name ' })

Call log:
  - waiting for locator('input[name="testName"]')
...
```ts
  ...
  10 | 
  11 |     //await page.waitForTimeout(9000);
  12 |     //await page.getByRole('button', {type: 'submit'}).click();
  13 |     await page.locator('.contact-toggler').click();
  14 |     await page.getByPlaceholder('Enter Your Name').fill('Test');
  15 |     await page.getByPlaceholder('Your Mobile Number').fill('8888888888');
> 16 |      await page.locator('input[name="testName"]').fill('Your value');
     |                                                   ^ Error: locator.fill: Error: strict mode violation: locator('input[name="testName"]') resolved to 2 elements:
